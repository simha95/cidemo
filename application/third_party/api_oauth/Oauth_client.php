...
class Oauth_client
{
    ...
    Function SendAPIRequest($url = '', $method, $parameters, $oauth, $options, &$response)
    {
        if ($url != '') {
            ...
            $query_arr = explode("&", $querystring);
            if (count($query_arr) > 0) {
                $temparr = array();
                foreach ($query_arr as $vals) {
                    if (!empty($vals)) {
                        $new_inner_arr = explode('=', $vals);
                        if (!in_array($new_inner_arr[0], $temparr)) {
                            $temparr[$new_inner_arr[0]] = $new_inner_arr[1];
                        }
                    }
                }
            }
            $url = $urlstring['scheme'] . '://' . $urlstring['host'] . $urlstring['path'] . '?' . http_build_query($temparr);
        }
...
//		$http = new http_class;
        ...
        $this->CI->http->redirection_limit = (IsSet($options['FollowRedirection']) ? intval($options['FollowRedirection']) : 0);
        $this->CI->http->follow_redirect = ($this->CI->http->redirection_limit != 0);
        if ($this->debug)
...
//            echo $this->authorization_header;exit;
...
//        print_r($arguments);die;
    ...
    Function ProcessToken($code, $refresh)
    {
        ...
        if (!$this->SendAPIRequest($access_token_url, 'POST', $values, null, $options, $response, $this->server)) {
            return false;
        }
    ...
    Function CallAPICustom($url, $method, $parameters, $options, $header_params_query_arr, &$response)
    {
        if (!IsSet($options['Resource']))
            $options['Resource'] = 'API call';
        if (!IsSet($options['ConvertObjects']))
        ...
        return($this->SendAPIRequestBypass($url, $method, $parameters, $oauth, $options, $header_params_query_arr, $response));
    }

    Function SendAPIRequestBypass($url = '', $method, $parameters, $oauth, $options, $header_params_query_arr, &$response)
    {
...
//            echo $this->authorization_header;exit;
        ...
        if (!empty($header_params_query_arr)) {
            foreach ($header_params_query_arr as $key => $val) {
                $arguments['Headers'][$key] = urldecode(trim($val));
            }
//            foreach ($header_params_query_arr)
        }
        if (strlen($error = $this->CI->http->SendRequest($arguments)) || strlen($error = $this->CI->http->ReadReplyHeaders($headers))) {
            $this->CI->http->Close();
            return($this->SetError('it was not possible to retrieve the ' . $options['Resource'] . ': ' . $error));
    ...
    Function Initialize($db_select_details)
    {
        ...
        $this->oauth_version = $db_select_details['vOauthVersion'];
        $this->dialog_url = $db_select_details['vDialogueURL'];
        $this->access_token_url = $db_select_details['vAccessTokenURL'];
        $this->url_parameters = $db_select_details['eURLParameters'];
        $this->authorization_header = $db_select_details['eAuthorizationHeader'];
        $this->request_token_url = $db_select_details['vRequestTokenURL'];
        $this->token_request_method = $db_select_details['eTokenRequestMethod'];
        $this->access_token_authentication = $db_select_details['vAccessTokenAuthentication'];
        ...
        /* 		
          ...
          default:
          $this->configuration_file = $inc_class_path . 'oauth/' . $this->configuration_file;
                ...
                if (IsSet($access_token['authorized']) && IsSet($access_token['value'])) {
                    ...
                    if (!$access_token['authorized'] || $expired) {
                        ...
                        elseif ($token !== $access_token['value']) {
                            ...
                            if (!$this->SendAPIRequest($url, $method, array(), $oauth, $options, $response, $this->server))
                                return false;
                            if (strlen($this->access_token_error)) {
                                $this->authorization_error = $this->access_token_error;
                                return true;
                    ...
                    $this->error = 'The OAuth access token is not set';
    ...
    Function getFullToken($flow_id, $method, $parameters, $options, &$response, $service_url = '')
    {
        ...
        switch (intval($this->oauth_version)) {
            ...
            case 2:
                ...
                $oauth = null;
                if (strcasecmp($this->access_token_type, 'Bearer')) {
                    $_SESSION["flow_" . $flow_id][(strlen($this->access_token_parameter) ? $this->access_token_parameter : 'access_token')] = UrlEncode($this->access_token);
        ...
        return($this->SendAPIRequestCustom($url, $method, $parameters, $oauth, $options, $response, $flow_id, $service_url));
    }

    Function SendAPIRequestCustom($url = '', $method, $parameters, $oauth, $options, &$response, $flow_id, $service_url = '')
    {
        ...
        switch (IsSet($options['AccessTokenAuthentication']) ? strtolower($options['AccessTokenAuthentication']) : '') {
            case 'basic':
                $arguments['Headers']['Authorization'] = 'Basic ' . base64_encode($this->client_id . ':' . ($this->get_token_with_api_key ? $this->api_key : $this->client_secret));
                $_SESSION["flow_" . $flow_id]['access_token'] = 'Basic ' . base64_encode($this->client_id . ':' . ($this->get_token_with_api_key ? $this->api_key : $this->client_secret));
                return "token_authenticated";
                break;
            case '':
                if (strlen($authorization)) {
                    $arguments['Headers']['Authorization'] = $authorization;
                    $_SESSION["flow_" . $flow_id]['access_token'] = $authorization;
                    if ($method === 'GET') {
                        $_SESSION["flow_" . $flow_id]['url_patch'] = $temp_url;
                    } else {
                        $_SESSION["flow_" . $flow_id]['url_patch'] = $post_values;
                    }

                    return "token_authenticated";
                } else {
                    $_SESSION["flow_" . $flow_id]['access_token'] = $this->access_token;
                    $_SESSION["flow_" . $flow_id]['url_patch'] = $post_values;
                    return "token_authenticated";
                }
                break;
