...
/*
  ...
  (c)1999 - 2003 All copyrights by: Danidl Eiland
...
class ImageMagick
{
    ...
    public function __construct($imagemagickinstalldir, $temp_gallery)
    {
        $this->temp_dir = $temp_gallery;
        if(!empty($imagemagickinstalldir)){
           $imagemagickinstalldir = rtrim($imagemagickinstalldir."/")."/";
        }
     ...
     */
    public function loadByFilePath($filepath)
    {
     ...
     */
    public function loadByFileData($filedata)
    {
     ...
     */
    public function Flip($var = 'horizontal')
    {
        if ($this->verbose == TRUE) {
            echo "Flip:\n";
        }
     ...
     */
    public function Rotate($deg = 90, $bgcolor = '000000', $how = '')
    {
    ...
    /*
      ...
      arg2 > sigma
     */
    public function Blur($radius = 5, $sigma = 2)
    {
        if ($this->verbose == TRUE) {
            echo "Blur:\n";
            echo "  Radius: {$radius}\n";
     ...
     */
    public function Frame($width = 6, $color = '666666')
    {
        if ($this->verbose == TRUE) {
            echo "Frame:\n";
            echo "  Width: {$width}\n";
    ...
    /*
      FrameCustom(int value, int value, sting string)
      Draws a frame around the image
      arg1 > frame width in pixels
      arg2 > frame height in pixels
      arg3 > frame color in hexadecimal, for exaple: 4AF2C9
     */
    public function FrameCustom($width = 6, $height = 0, $color = '666666')
    {
        if ($this->verbose == TRUE) {
            echo "Frame:\n";
            echo "  Width: {$width}\n";
        ...
        $command = "{$this->imagemagickdir}convert -mattecolor \"#{$color}\" -frame \"{$width}x{$height}\" \"{$this->temp_dir}/tmp{$this->count}_{$this->temp_file}\" \"{$this->temp_dir}/tmp" . ++$this->count . "_{$this->temp_file}\"";
        if ($this->verbose == TRUE) {
            echo "  Command: {$command}\n";
        }
        exec($command, $returnarray, $returnvalue);
     ...
     */
    public function Crop($size_x, $size_y, $how = 'center')
    {
