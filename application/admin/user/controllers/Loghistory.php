...
class Loghistory extends Cit_Controller
{
    ...
    public function index()
    {
        ...
        list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access) = $this->filter->getModuleWiseAccess("loghistory", array("List", "View", "Add", "Update", "Delete", "Export"), TRUE, TRUE);
        try
        {
            if (!$list_access)
            {
    ...
    public function export()
    {
        ...
        $heading = "Log History";
        $filename = "Log_History_".count($db_recs)."_Records";
    ...
    public function add()
    {
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                list($list_access, $view_access, $edit_access, $del_access, $expo_access) = $this->filter->getModuleWiseAccess("loghistory", array("List", "View", "Update", "Delete", "Export"), TRUE, TRUE);
            ...
            {
                list($list_access, $add_access, $del_access) = $this->filter->getModuleWiseAccess("loghistory", array("List", "Add", "Delete"), TRUE, TRUE);
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                ...
                $switch_arr = $this->loghistory_model->getSwitchTo($extra_cond, "records", $this->switchto_limit);
                $switch_combo = $this->filter->makeArrayDropDown($switch_arr);
                $switch_cit = array();
                $switch_tot = $this->loghistory_model->getSwitchTo($extra_cond, "count");
                if ($this->switchto_limit > 0 && $switch_tot > $this->switchto_limit)
                {
                    ...
                    {
                        $extra_cond = $this->db->protect($this->loghistory_model->table_alias.".".$this->loghistory_model->primary_key)." = ".$this->db->escape($id);
                        $switch_cur = $this->loghistory_model->getSwitchTo($extra_cond, "records", 1);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            {
                                $tmpdata = $this->loghistory_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key);
                            }
                        }
