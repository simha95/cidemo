...
//CIT Customization
if ($_COOKIE['ci_rfm_action'] != $_GET['mkey']) {
...
if (!is_dir($thumbs_path.$subdir) && $subdir != '')
{
...
//CIT Customization
$rfm_action = time();
setcookie("ci_rfm_action", $rfm_action, time() + 3600, '/');
...
<html xmlns="https://www.w3.org/1999/xhtml">
	...
	<script>
        ...
        var lang_Error_extension = '<?php echo trans('Error_extension');?>';
        var lang_Error_Upload = '<?php echo trans('Error_Upload');?>';
        var lang_Upload_Message = '<?php echo trans('Upload_message');?>';
        var lang_Error_Result = '<?php echo trans('SERVER ERROR');?>';
        var maxFilesize = <?php echo $MaxSizeUpload;?>;
        var apply = '<?php echo $apply; ?>';
		...
		<div class="tab-content">
			<div class="tab-pane active" id="tab1">
				...
				<form action="dialog.php" method="post" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
					...
					<input type="hidden" name="path_thumb" value="<?php echo $thumbs_base_path.$subdir?>"/>
					<div class="fallback">
		...
		foreach ($files as $file_array) {
			...
			?>
				...
				<?php if($file==".."){ ?>
					...
					<input type="hidden" class="path_thumb" value="<?php echo dirname($thumbs_base_path.$subdir)."/";?>"/>
				<?php } ?>
			...
			$file=$file_array['file'];
                ...
                if($src_thumb != ""){
                    $mini_src = $src_thumb = $thumbs_current_path.$subdir. $file;
                }
				$is_img=true;
